...
func ReadConfig(UserMap map[string]*UserList){
	jsonPath:="../res/userInfo.json"
...
func GetSessionData(UserMap map[string]*UserList) (session map[string]string){
	session = make(map[string]string)
	url := "https://yjsxx.whut.edu.cn/wx/api/login/checkBind"
	f, err := os.OpenFile("./res/session_backup.txt", os.O_WRONLY|os.O_TRUNC, 0600)
	if err != nil {
...
func main(){
	...
	ReadConfig(UserMap)
	SeesionMap := GetSessionData(UserMap)
	BindUserInfo(UserMap,SeesionMap)
	Report(UserMap,SeesionMap)
	CancelBind(UserMap,SeesionMap)
}